...
import com.fileuploader.app.IntegrationTest;
import com.fileuploader.app.domain.Content;
import com.fileuploader.app.domain.Document;
...
class ContentResourceIT {
  ...
  @Autowired private MockMvc restContentMockMvc;
  ...
  void createContent() throws Exception {
    ...
    assertThat(testContent.getDataContentType())
        .isEqualTo(DEFAULT_DATA_CONTENT_TYPE);
  }
  ...
  void getAllContents() throws Exception {
    ...
    restContentMockMvc.perform(get(ENTITY_API_URL + "?sort=id,desc"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(
            jsonPath("$.[*].id").value(hasItem(content.getId().intValue())))
        .andExpect(jsonPath("$.[*].dataContentType")
  ...
  void getContent() throws Exception {
    ...
    // Get the content
    restContentMockMvc.perform(get(ENTITY_API_URL_ID, content.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(jsonPath("$.id").value(content.getId().intValue()))
        .andExpect(
            jsonPath("$.dataContentType").value(DEFAULT_DATA_CONTENT_TYPE))
        .andExpect(
            jsonPath("$.data").value(Base64Utils.encodeToString(DEFAULT_DATA)));
  }
  ...
  void getNonExistingContent() throws Exception {
    // Get the content
    restContentMockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE))
  ...
  void putExistingContent() throws Exception {
    ...
    // Disconnect from session so that the updates on updatedContent are not
    // directly saved in db
    em.detach(updatedContent);
    updatedContent.data(UPDATED_DATA)
        .dataContentType(UPDATED_DATA_CONTENT_TYPE);
    ...
    assertThat(testContent.getDataContentType())
        .isEqualTo(UPDATED_DATA_CONTENT_TYPE);
  }
  ...
  void putNonExistingContent() throws Exception {
    ...
    restContentMockMvc
        .perform(put(ENTITY_API_URL_ID, content.getId())
                     .contentType(MediaType.APPLICATION_JSON)
                     .content(TestUtil.convertObjectToJsonBytes(content)))
        .andExpect(status().isBadRequest());
  ...
  void partialUpdateContentWithPatch() throws Exception {
    ...
    restContentMockMvc
        .perform(patch(ENTITY_API_URL_ID, partialUpdatedContent.getId())
  ...
  void patchNonExistingContent() throws Exception {
    ...
    // If the entity doesn't have an ID, it will throw BadRequestAlertException
    restContentMockMvc
        .perform(patch(ENTITY_API_URL_ID, content.getId())
                     .contentType("application/merge-patch+json")
                     .content(TestUtil.convertObjectToJsonBytes(content)))
        .andExpect(status().isBadRequest());
  ...
  void deleteContent() throws Exception {
    ...
    // Delete the content
    restContentMockMvc
        .perform(delete(ENTITY_API_URL_ID, content.getId())
